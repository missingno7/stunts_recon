...
root=Path('.').resolve(); case=root/'recovery/corpus/endurance-001/is_facing_camera'; out=case/'trials'/'002'; out.mkdir(parents=True,exist_ok=False)
src=case/'candidates'/'002.c'; pred=case/'predictions'/'001.json'; source=src.read_bytes(); prediction=read_json(pred)
...
print(json.dumps({'strict':strict,'effective_code':receipt.get('effective_code'),'diagnosis':d.get('match_summary')},indent=2)); print(format_summary(d.get('match_summary'),'camera hypothesis 5'))
