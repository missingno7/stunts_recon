...
def capabilities(function, disassembly, known_data_offsets):
    ...
    for row in disassembly:
        ...
        # Only this straight-line AX=CS -> segment-register transfer is recognized.
        # A branch invalidates the observation; this is not general dataflow.
        for segment in ('ds', 'es'):
            if text.startswith(('mov '+segment+',', 'pop '+segment)):
                cs_loaded_segments.discard(segment)
                if text == 'mov '+segment+', ax' and previous == 'mov ax, cs':
                    cs_loaded_segments.add(segment)
        if text.startswith(('rep movsb', 'rep movsw', 'rep stosb', 'rep stosw')) and cs_loaded_segments:
            blockers.append('CS-relative storage is unsupported')
        ...
        # LEA computes an address; its operand is not a default-DS memory read.
        for offset in re.findall(r'(?<!:)\[(0x[0-9a-f]+)\]', text):
            if int(offset, 16) not in known_data_offsets:
                blockers.append(('Unregistered absolute address expression ' if mnemonic == 'lea'
                                 else 'Unregistered DGROUP address ')+offset)
