...
The cohort's 618-byte car-state initializer adds a different scoped compiler response: moving the declaration of a zero-valued register local changed the complete frame/register output, and reversing a chained assignment changed the order of two adjacent byte stores. Hypothesis 22 then matched the whole original contribution and passed native promotion. An arithmetic `>>6` also removed the `__aFldiv` fixups produced by a Restunts-style `/64` expression; the two expressions can differ for negative coordinates, so this byte-exact result is not proof of unique historical source or semantic equivalence. Preserve operand order and type questions when carrying this pattern to another initializer; see the [cohort receipts](../recovery/corpus/ordinary-c-cohort-20260924/results.md).
