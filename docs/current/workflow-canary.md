...
The production candidate remains `recovery/candidates/is_facing_camera.c`. It preserves the important width asymmetry: X operands widen before subtraction, Y subtraction wraps at 16 bits then sign-extends, and products/determinant use the compiler's 32-bit helper. It emits 194 bytes. After binding, the first mismatch is **+0x37**: `mov [bp-8],ax` instead of `mov [bp-0xC],ax`. Its dx1/dy0 allocation differs, followed by the final boolean/return block; the complete target is six bytes longer. `recovery/canary-bound-diagnostic.json` proves both source call operands/relocation sites can be bound while retaining this failure.

A later desktop research source declares `dx0, dx1, dy0, dy1` and experimentally selects the target's paired BP homes. A controlled two-source batch found 11 changed common byte positions, unchanged declarations/fixups, 35 gained exact target anchor bytes and no lost anchors. This is a scoped compiler response, not a strict match: the contribution remains 194 bytes and first differs at the +0x4A branch destination. A bounded negative-polarity follow-up also emitted 194 bytes and lost late target anchors without reproducing the AL/CBW return path. Combining the declaration order with a byte-result cast emitted 198 bytes and AL/CBW, but retained the wrong early zero-return topology and lacked the target trailing false epilogue/NOP sequence. The exact sources, predictions and diagnostic comparisons are in `recovery/experiments/is_facing_camera/declaration-order-20260923.md`; production remains blocked.
