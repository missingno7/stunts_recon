...
Start with `python tools/context.py is_facing_camera --history` and `workflow-canary.md`. Its true 200-byte pristine extent and bounded far-call capability are now proven; the ordinary-C hypothesis still emits 194 bytes. The family is explicitly blocked after the documented source/profile/TU experiments. Reopen only for new source-layout or boolean-lowering evidence, not another arbitrary flag sweep.
